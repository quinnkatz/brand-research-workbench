...
export async function POST(req: Request) {
  try {
    const uid = await owner(req, true);
...
export async function GET(req: Request) {
  try {
    ...
    const row = await db().prepare("SELECT * FROM attachments WHERE id = ? AND owner_id = ?").bind(id, uid).first<Record<string, string>>();
