...
import { protocolOf,stableJson } from "./comparisons";
import { queueDigest, deliverNotification } from "./notifications";
...
export async function afterCollection(ownerId:string,runId:string) {
  ...
  const previous=previousRows.results.map(publicRun).find(r=>eligible(r)&&stableJson(protocolOf(r))===stableJson(protocolOf(current)));
  ...
  const monitor=await db().prepare("SELECT * FROM monitors WHERE id=? AND owner_id=?").bind(monitorId,ownerId).first<any>();if(!monitor)return;
  const existing=await db().prepare("SELECT id FROM report_snapshots WHERE id=?").bind(batchId).first();
  if(JSON.parse(monitor.config).report&&!existing){
    ...
    const createdAt=new Date().toISOString(),title=`${monitor.name} · ${createdAt.slice(0,10)}`,key=`${ownerId}/reports/${batchId}/${crypto.randomUUID()}.json`;
    ...
    const saved=await db().prepare("INSERT OR IGNORE INTO report_snapshots (id,owner_id,study_id,title,token_hash,object_key,created_at,expires_at) VALUES (?,?,?,?,?,?,?,?)").bind(batchId,ownerId,current.study_id,title,await hash(crypto.randomUUID()),key,createdAt,new Date(Date.now()+30*86400000).toISOString()).run();
    if(!saved.meta.changes)await bucket().delete(key);
  }
  const report=await db().prepare("SELECT id FROM report_snapshots WHERE id=? AND owner_id=?").bind(batchId,ownerId).first();
  const notificationId=await queueDigest(monitor,batchId,study,!!report);
  if(notificationId)try{await deliverNotification(notificationId,ownerId);}catch{/* Saved delivery remains visible and recoverable without repeating a provider collection. */}
}
